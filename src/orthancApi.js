...
export default {
  ...
  async sendToDicomWebServer(resourcesIds, destination) {
    const response = await axios.post(
      orthancApiUrl + 'dicom-web/servers/' + destination + '/stow',
      {
  ...
  async sendToOrthancPeer(resourcesIds, destination) {
    const response = await axios.post(
      orthancApiUrl + 'peers/' + destination + '/store',
      {
        Resources: resourcesIds,
        Synchronous: false,
      },
  ...
  async sendToDicomModality(resourcesIds, destination) {
    const response = await axios.post(
      orthancApiUrl + 'modalities/' + destination + '/store',
      {
        Resources: resourcesIds,
        Synchronous: false,
      },
  ...
  async deleteResource(level, orthancId) {
    return axios.delete(
      orthancApiUrl + this.pluralizeResourceLevel(level) + '/' + orthancId,
    );
  },
  ...
  async findStudies(filterQuery, labels, LabelsConstraint) {
    ...
    return (
      await axios.post(orthancApiUrl + 'tools/find', payload, {
  ...
  async getChanges(since, limit) {
    const response = await axios.get(
      orthancApiUrl + 'changes?since=' + since + '&limit=' + limit,
    );
    return response.data;
  },
  ...
  async findPatient(patientId) {
    const response = await axios.post(
      orthancApiUrl + 'tools/lookup',
      patientId,
    );
    if (response.data.length == 1) {
      const patient = await axios.get(
        orthancApiUrl + 'patients/' + response.data[0]['ID'],
      );
      return patient.data;
    } else {
      return null;
  ...
  async findStudy(studyInstanceUid) {
    ...
    if (response.data.length == 1) {
      const study = await axios.get(
        orthancApiUrl + 'studies/' + response.data[0]['ID'],
      );
      return study.data;
    } else {
      return null;
  ...
  async mergeSeriesInExistingStudy({ seriesIds, targetStudyId, keepSource }) {
    const response = await axios.post(
      orthancApiUrl + 'studies/' + targetStudyId + '/merge',
      {
        Resources: seriesIds,
  ...
  async remoteDicomFindStudies(remoteModality, filterQuery) {
    ...
    try {
      const queryResponse = (
        await axios.post(
          orthancApiUrl + 'modalities/' + remoteModality + '/query',
          {
            Level: 'Study',
            Query: filterQuery,
          },
  ...
  async remoteDicomRetrieveStudy(
    remoteModality,
    filterQuery,
    targetAet,
    level,
    ...
    const response = await axios.post(
      orthancApiUrl + 'modalities/' + remoteModality + '/move',
      {
        Level: level,
  ...
  async remoteModalityEcho(remoteModality) {
    return axios.post(
      orthancApiUrl + 'modalities/' + remoteModality + '/echo',
      {},
    );
  },
  ...
  async getStudySeries(orthancId) {
    return (await axios.get(orthancApiUrl + 'studies/' + orthancId + '/series'))
      .data;
  },
  async getSeriesInstances(orthancId) {
    return (
      await axios.get(orthancApiUrl + 'series/' + orthancId + '/instances')
    ).data;
  },
  async getStudyInstances(orthancId) {
    return (
      await axios.get(orthancApiUrl + 'studies/' + orthancId + '/instances')
    ).data;
  },
  async getSeriesParentStudy(orthancId) {
    return (await axios.get(orthancApiUrl + 'series/' + orthancId + '/study'))
      .data;
  },
  async getInstanceParentStudy(orthancId) {
    return (
      await axios.get(orthancApiUrl + 'instances/' + orthancId + '/study')
    ).data;
  },
  ...
  async getInstanceTags(orthancId) {
    return (await axios.get(orthancApiUrl + 'instances/' + orthancId + '/tags'))
      .data;
  },
  async getSimplifiedInstanceTags(orthancId) {
    return (
      await axios.get(
        orthancApiUrl + 'instances/' + orthancId + '/tags?simplify',
      )
    ).data;
  },
  async getInstanceHeader(orthancId) {
    return (
      await axios.get(orthancApiUrl + 'instances/' + orthancId + '/header')
    ).data;
  },
  async getStatistics() {
  ...
  async generateUid(level) {
    return (
      await axios.get(orthancApiUrl + 'tools/generate-uid?level=' + level)
    ).data;
  },
  ...
  async anonymizeResource({
    ...
    const response = await axios.post(
      orthancApiUrl +
        this.pluralizeResourceLevel(resourceLevel) +
        '/' +
        orthancId +
        '/anonymize',
      {
  ...
  async addLabel({ studyId, label }) {
    await axios.put(
      orthancApiUrl + 'studies/' + studyId + '/labels/' + label,
      '',
    );
    return label;
  },
  ...
  async getLabels(studyId) {
    const response = await axios.get(
      orthancApiUrl + 'studies/' + studyId + '/labels',
    );
    return response.data;
  },
  ...
  async createToken({
    ...
    const response = await axios.put(
      orthancApiUrl + 'auth/tokens/' + tokenType,
      body,
    );
  ...
  getVolViewUrl(level, resourceOrthancId) {
    const volViewVersion =
      store.state.configuration.installedPlugins.volview.Version;
    const urls =
      'urls=[../' +
      this.pluralizeResourceLevel(level) +
      '/' +
      resourceOrthancId +
      '/archive]';
  ...
  getStoneViewerUrlForBulkStudies(studiesDicomIds) {
    return (
      ...
      studiesDicomIds.join(',')
    );
  },
  getOhifViewerUrlForDicomJson(mode, resourceOrthancId) {
    if (mode == 'basic') {
      return (
        store.state.configuration.uiOptions.OhifViewer3PublicRoot +
        'viewer?url=../studies/' +
        resourceOrthancId +
        '/ohif-dicom-json'
      );
      ...
      return (
        store.state.configuration.uiOptions.OhifViewer3PublicRoot +
        'viewer?hangingprotocolId=mprAnd3DVolumeViewport&url=../studies/' +
        resourceOrthancId +
        '/ohif-dicom-json'
      );
    } else if (mode == 'tmtv') {
  ...
  getOhifViewerUrlForDicomWeb(mode, resourceDicomUid) {
    if (store.state.configuration.uiOptions.EnableOpenInOhifViewer3) {
      if (mode == 'basic') {
        ...
        return (
          store.state.configuration.uiOptions.OhifViewer3PublicRoot +
          'viewer?hangingprotocolId=mprAnd3DVolumeViewport&StudyInstanceUIDs=' +
          resourceDicomUid
        );
      } else if (mode == 'tmtv') {
        return (
          store.state.configuration.uiOptions.OhifViewer3PublicRoot +
          'tmtv?StudyInstanceUIDs=' +
          resourceDicomUid
        );
      }
    } else {
      return (
        store.state.configuration.uiOptions.OhifViewerPublicRoot +
        'Viewer/' +
        resourceDicomUid
      );
    }
