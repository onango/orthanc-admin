...
const setGetConfig = (config) => {
  ...
  const accessToken =
    store.getters.token && store.getters.token.accessToken
      ? store.getters.token.accessToken
      : '';
  ...
  (response) => {
    ...
    if (response.data.errno === 0) {
      ...
      if (response.data.errno === 4001) {
        ...
        if (!isRefreshing) {
          const refreshToken =
            store.getters.token && store.getters.token.refreshToken
              ? store.getters.token.refreshToken
              : '';
          ...
          return store
            ...
            .then(() => {
              ElMessage.success(
                'Refresh access token success, retry pending requests...',
              );
              console.debug(
                'Refresh access token success, requests:',
                requests,
              );
              isRefreshing = false;
              refreshSuccess = true;
