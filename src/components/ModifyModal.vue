  ...
  methods: {
    async reset() {
      ...
      this.samePatientStudiesCount = (
        await api.getSamePatientStudies(this.patientMainDicomTags['PatientID'])
      ).length;
    ...
    async monitorJob(jobId) {
      ...
      this.jobIsComplete =
        jobStatus.State == 'Success' || jobStatus.State == 'Failure';
      ...
      if (!this.jobIsComplete) {
        ...
        if (jobType == 'MergeStudy') {
          const modifiedStudy = await api.getStudy(
            jobStatus['Content']['TargetStudy'],
          );
          this.showModifiedResourceValue =
            modifiedStudy['MainDicomTags']['StudyInstanceUID'];
          ...
          if (jobStatus['Content']['Type'] == 'Patient') {
            ...
            const modifiedPatient = await api.getPatient(
              jobStatus['Content']['ID'],
            );
            this.showModifiedResourceValue =
              modifiedPatient['MainDicomTags']['PatientID'];
            ...
            this.showModifiedResourceKey = 'StudyInstanceUID';
            const modifiedStudy = await api.getSeriesParentStudy(
              jobStatus['Content']['ID'],
            );
            this.showModifiedResourceValue =
              modifiedStudy['MainDicomTags']['StudyInstanceUID'];
      ...
      try {
        ...
        if (this.action == 'attach-study-to-existing-patient') {
          ...
          if (targetPatient) {
            ...
            console.log(
              'attach-study-to-existing-patient: created job ',
              jobId,
            );
            this.startMonitoringJob(jobId);
          } else {
            console.error(
              'Error while changing patient, the new PatientID does not exist ',
              this.tags['PatientID'],
            );
            this.setError(
              'modify.error_attach_study_to_existing_patient_target_does_not_exist_html',
            );
            return;
          }
          ...
          if ('PatientID' in this.modifiedTags) {
            ...
            if (targetPatient) {
              console.error(
                'modify-any-tags-in-one-study: Error while modifying patient-study tags, another patient with the same PatientID already exists ',
                this.tags['PatientID'],
                targetPatient,
              );
              this.setError(
                'modify.error_modify_any_study_tags_patient_exists_html',
              );
              return;
            }
          } else if (
            'PatientName' in this.modifiedTags ||
            'PatientBirthDate' in this.modifiedTags ||
            'PatientSex' in this.modifiedTags
          ) {
            console.log(
              'modify-any-tags-in-one-study: Patient tags have changed',
            );
            this.samePatientStudiesCount = (
              await api.getSamePatientStudies(this.originalTags['PatientID'])
            ).length;
            if (this.samePatientStudiesCount > 1) {
              console.error(
                'modify-any-tags-in-one-study: Error while modifying patient-study tags, this patient has other studies, you can not modify patient tags ',
                this.originalTags['PatientID'],
                this.modifiedTags,
              );
              this.setError(
                'modify.error_modify_any_study_tags_can_not_modify_patient_tags_because_of_other_studies_html',
              );
              return;
            } else {
              console.log(
                'modify-any-tags-in-one-study: this is the only patient study, we can modify all tags',
              );
            }
          }
          ...
          if (this.tags['PatientID'] != this.originalTags['PatientID']) {
            ...
            if (targetPatient) {
              console.error(
                'modify-patient-tags-in-all-studies: Error while modifying patient tags, another patient with the same PatientID already exists ',
                this.tags['PatientID'],
                targetPatient,
              );
              this.setError(
                'modify.error_modify_patient_tags_another_patient_exists_with_same_patient_id_html',
              );
              return;
            }
          }

          const originalPatient = await api.findPatient(
            this.originalTags['PatientID'],
          );
          const jobId = await api.modifyResource({
          ...
          console.log(
            'modify-patient-tags-in-all-studies: created job ',
            jobId,
          );
          this.startMonitoringJob(jobId);
          ...
          if (
            ...
            this.originalTags['StudyInstanceUID']
          ) {
            console.error(
              'attach-series-to-existing-study: Error target study is same as current study ',
              this.tags['StudyInstanceUID'],
            );
            this.setError(
              'modify.error_attach_series_to_existing_study_target_unchanged_html',
            );
            return;
          } else {
            const targetStudy = await api.findStudy(
              this.tags['StudyInstanceUID'],
            );
            if (targetStudy == null) {
              console.error(
                'attach-series-to-existing-study: target study not found ',
                this.tags['StudyInstanceUID'],
              );
              this.setError(
                'modify.error_attach_series_to_existing_study_target_does_not_exist_html',
              );
              return;
            } else {
              ...
              console.log(
                'attach-series-to-existing-study: created job ',
                jobId,
              );
              this.startMonitoringJob(jobId);
            }
          ...
          if (targetPatient) {
            if (!hasAcceptedWarning) {
              if (
                targetPatient['MainDicomTags']['PatientName'] !=
                  this.tags['PatientName'] ||
                targetPatient['MainDicomTags']['PatientBirthDate'] !=
                  this.tags['PatientBirthDate'] ||
                targetPatient['MainDicomTags']['PatientSex'] !=
                  this.tags['PatientSex']
              ) {
                console.warn(
                  'create-new-study-from-series: Another patient exists but tags differ',
                  targetPatient['MainDicomTags'],
                  this.tags,
                );
                this.setWarning(
                  'modify.warning_create_new_study_from_series_html',
                );
                return;
              }
            } else {
              console.log(
                'create-new-study-from-series: warning accepted, copying existing Patient tags',
              );
              this.tags['PatientName'] =
                targetPatient['MainDicomTags']['PatientName'];
              this.tags['PatientBirthDate'] =
        ...
        if (action == 'anonymize-study') {
          ...
          if ('StudyDescription' in this.studyMainDicomTags) {
            this.tags['StudyDescription'] =
              this.studyMainDicomTags['StudyDescription'];
          }
    ...
    isEditableTag(tag) {
      ...
      if (this.resourceLevel == 'series') {
        if (
          this.action == 'attach-series-to-existing-study' &&
    ...
    isDicomUid(tag) {
      return (
        ['StudyInstanceUID', 'SeriesInstanceUID', 'SOPInstanceUID'].indexOf(
          tag,
        ) != -1
      );
    },
  ...
  computed: {
    ...
    keepSource() {
      if (this.modificationMode == 'duplicate') {
        ...
        if (this.action == 'attach-study-to-existing-patient') {
          // PatientID changes -> orthancID changes -> delete original
          return false;
        } else if (
          this.action == 'modify-any-tags-in-one-study' ||
          this.action == 'modify-patient-tags-in-all-studies'
        ) {
          if ('PatientID' in this.modifiedTags) {
    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <h5 v-if="!isAnonymization" id="modalLabel" class="modal-title">
            {{ $t('modify.modify_modal_title') + ' ' + resourceTitle }}
          </h5>
          <h5 v-if="isAnonymization" id="modalLabel" class="modal-title">
            {{ $t('modify.anonymize_modal_title') + ' ' + resourceTitle }}
          </h5>
          <button
            type="button"
            class="btn-close"
        ...
        <div v-if="step == 'init'" class="modal-body">
          <div class="container">
            <div
              v-if="resourceLevel == 'study' && !isAnonymization"
            ...
            >
              <div
                ...
                v-html="
                  $t(
                    'modify.study_step_0_attach_study_to_existing_patient_html',
                  )
                "
              ></div>
              <div class="col-md-3">
                <button
                  ...
                  @click="
                    goToNextStep(step, 'attach-study-to-existing-patient')
                  "
                  v-html="
                    $t(
                      'modify.study_step_0_attach_study_to_existing_patient_button_title_html',
                    )
                  "
                ></button>
              </div>
            ...
            >
              <div
                class="col-md-9"
                v-html="
                  $t('modify.study_step_0_patient_has_other_studies_html', {
              ...
              <div class="col-md-3">
                <button
                  ...
                  class="btn btn-primary w-100"
                  @click="
                    goToNextStep(step, 'modify-patient-tags-in-all-studies')
                  "
                  v-html="
                    $t(
                      'modify.study_step_0_patient_has_other_studies_button_title_html',
                    )
                  "
                ></button>
              </div>
            ...
            >
              <div
                class="col-md-9"
                v-html="
                  $t('modify.study_step_0_modify_study_html', {
                    count: samePatientStudiesCount,
                  })
                "
              ></div>
              <div class="col-md-3">
                <button
                  ...
                  v-html="
                    $t('modify.study_step_0_modify_study_button_title_html')
                  "
                ></button>
              </div>
            </div>
            <div
              v-if="resourceLevel == 'series' && !isAnonymization"
              class="row border-bottom pb-3"
            >
              <div
                class="col-md-9"
                v-html="
                  $t(
                    'modify.series_step_0_attach_series_to_existing_study_html',
                  )
                "
              ></div>
              <div class="col-md-3">
                <button
                  type="button"
                  class="btn btn-primary w-100"
                  @click="goToNextStep(step, 'attach-series-to-existing-study')"
                  v-html="
                    $t(
                      'modify.series_step_0_attach_series_to_existing_study_button_title_html',
                    )
                  "
                ></button>
              </div>
            ...
            >
              ...
              <div class="col-md-3">
                <button
                  type="button"
                  class="btn btn-primary w-100"
                  @click="goToNextStep(step, 'create-new-study-from-series')"
                  v-html="
                    $t(
                      'modify.series_step_0_create_new_study_button_title_html',
                    )
                  "
                ></button>
              </div>
            ...
            >
              ...
              <div class="col-md-3">
                <button
                  type="button"
                  class="btn btn-primary w-100"
                  @click="goToNextStep(step, 'modify-series-tags')"
                  v-html="
                    $t('modify.series_step_0_modify_series_button_title_html')
                  "
                ></button>
              </div>
            </div>
        ...
        <div v-if="step == 'tags'" class="modal-body">
          <div class="container">
            <div v-for="(item, key) in tags" :key="key" class="row">
              ...
              <div
                class="col-md-5"
                :class="{ 'striked-through': isRemovedTag(key) }"
              >
                {{ key }}
              </div>
              ...
              <div v-if="isFrozenTag(key)" class="col-md-6">
                <input
                  ...
                  class="form-control"
                  disabled
                  :class="{ 'striked-through': !isDicomUid(key) }"
                />
              </div>
              ...
              <div v-if="true" class="col-md-1">
                <div class="d-flex flex-row-reverse">
                  <button
                    v-if="isRemovable(key) && !isRemovedTag(key)"
                    class="btn-small"
                    type="button"
                    data-bs-toggle="tooltip"
                    :title="$t('modify.remove_tag_tooltip')"
                  ...
                  <button
                    ...
                    type="button"
                    data-bs-toggle="tooltip"
                    :title="$t('modify.remove_tag_undo_tooltip')"
                    @click="toggleRemovedTag(key)"
                  >
              ...
              <div v-if="isAutogeneratedDicomUid(key)" class="col-md-1"></div>
            ...
            <div
              v-if="action == 'modify-any-tags-in-one-study'"
              class="row pt-3"
            >
            ...
            <div class="row">
              <div class="progress" style="width: 100%">
                <div
                  ...
                  role="progressbar"
                  :style="'width: ' + jobProgressComplete + '%'"
                ></div>
                <div
                  class="progress-bar bg-secondary"
                  role="progressbar"
                  :style="'width: ' + jobProgressRemaining + '%'"
                ></div>
                <div
                  class="progress-bar bg-danger"
                  role="progressbar"
                  :style="'width: ' + jobProgressFailed + '%'"
                ></div>
              </div>
            </div>
          </div>
        ...
        <div v-if="step == 'progress' || step == 'done'" class="modal-footer">
          ...
          <button
            ...
            :disabled="step != 'done'"
            data-bs-dismiss="modal"
            @click="showModifiedResources()"
          >
            {{ $t('modify.show_modified_resources') }}
          </button>
        </div>
      </div>
    </div>
