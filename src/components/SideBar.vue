  ...
  computed: {
    ...
    hasQueryableDicomModalities() {
      return (
        ...
        this.queryableDicomModalities.length > 0
      );
    },
    hasAccessToSettings() {
      ...
      <ul id="menu-content" class="menu-content collapse out">
        ...
        <ul v-if="allLabels.length > 0" class="sub-menu" id="labels-list">
          <li
            ...
            :key="label"
            v-bind:class="{ active: isSelectedLabel(label) }"
        ...
        >
          <i class="fa fa-radiation fa-lg menu-icon"></i
          >{{ $t('dicom_modalities') }}
        ...
        >
          <li
            v-for="modality in queryableDicomModalities"
            :key="modality"
          ...
          >
            <router-link
              ...
              :to="{
                ...
                query: { remoteMode: 'dicom', remoteSource: modality },
              }"
            >
              {{ modality }}
            </router-link>
            <span
              v-if="this.isEchoRunning(modality)"
              class="ms-auto spinner-border spinner-border-sm"
              data-bs-toggle="tooltip"
              title="Checking connectivity"
            ></span>
            <span v-else-if="this.isEchoSuccess(modality)" class="ms-auto"
              ><i
        ...
        <li
          ...
          class="d-flex align-items-center fix-router-link"
        >
        ...
        <li
          v-if="uiOptions.EnableLinkToLegacyUi"
          class="d-flex align-items-center fix-router-link"
        >
        ...
        >
          ...
          ><span v-if="hasUserProfile">{{ userProfile.name }}</span
          ><span v-if="!hasUserProfile">{{ $t('profile') }}</span>
          <span class="arrow ms-auto"></span>
        </li>
        <ul
          class="sub-menu collapse"
          id="profile-list"
          ref="profile-collapsible"
        >
          <li
            v-if="uiOptions.EnableChangePassword"
            class="d-flex align-items-center fix-router-link"
          >
            <a v-bind:href="'#'" @click="changePassword($event)">
              <i class="fa fa-solid fa-key fa-lg menu-icon"></i
              >{{ $t('change_password') }} </a
            ><span class="ms-auto"></span>
          </li>
          <li
          ...
          >
            <a v-bind:href="'#'" @click="logout($event)">
              <i
                class="fa fa-solid fa-arrow-right-from-bracket fa-lg menu-icon"
              ></i
              >{{ $t('logout') }} </a
            ><span class="ms-auto"></span>
          </li>
