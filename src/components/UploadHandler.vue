...
export default {
  ...
  mounted() {
    ...
    this.uppie(
      document.querySelector('#foldersUpload'),
      this.uppieUploadHandler,
    );
  },
  methods: {
    onDrop(ev) {
    ...
    async uploadedFile(uploadId, uploadedFileResponse) {
      ...
      if (!this.lastUploadReports[uploadId].uploadedStudiesIds.has(studyId)) {
        ...
        this.lastUploadReports[uploadId].uploadedStudies[studyId] =
          studyResponse;
        ...
        try {
          ...
          if (Array.isArray(uploadResponse)) {
            ...
            if (uploadResponse.length > 0) {
              ...
              this.lastUploadReports[uploadId].errorMessages[filename] =
                'no valid DICOM files found in zip';
            }
          } else {
            this.lastUploadReports[uploadId].successFilesCount++;
...
<template>
  <div class="upload">
    <div
      class="upload-handler-drop-zone"
      @drop="this.onDrop"
