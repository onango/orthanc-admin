  ...
  watch: {
    labelsModel: {
      async handler(oldValue, newValue) {
        const changed = await api.updateLabels({
  ...
  <table class="table table-responsive table-sm study-details-table">
    <tr v-if="uiOptions.EnableEditLabels">
      <td colspan="100%">
        ...
        >
          <option
            v-for="label in allLabelsLocalCopy"
            :key="label"
            :value="label"
    ...
    <tr>
      ...
      <td width="40%" class="cut-text">
        ...
        <p
          v-if="
            hasLoadedSamePatientsStudiesCount && samePatientStudiesCount > 1
          "
        >
          {{
          ...
          <router-link
            :to="
              '/filtered-studies?PatientID=&quot;' +
              patientMainDicomTags['PatientID'] +
              '&quot;'
            "
          >
