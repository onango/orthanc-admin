...
document._datePickerPresetRanges = [
  ...
  {
    tLabel: 'date_picker.yesterday',
    range: [subDays(new Date(), 1), subDays(new Date(), 1)],
  },
  {
    tLabel: 'date_picker.this_week',
    range: [startOfWeek(new Date()), new Date()],
  },
  {
    tLabel: 'date_picker.last_week',
    range: [
  ...
  {
    tLabel: 'date_picker.this_month',
    range: [startOfMonth(new Date()), endOfMonth(new Date())],
  },
  {
    tLabel: 'date_picker.last_month',
    range: [
  ...
  {
    tLabel: 'date_picker.last_12_months',
...
export default {
  ...
  async created() {
    ...
    //---Custom change to load on soft refresh --antony
    this.filterLabels = [];
  ...
  methods: {
    ...
    _updateLabelsFilter(labels) {
      this.$store.dispatch('studies/updateLabelsFilterNoReload', {
        labels: labels,
      });
      this.updateUrl();
    ...
    async updateFilterFromRoute(filters) {
      ...
      for (const [filterKey, filterValue] of Object.entries(filters)) {
        if (filterKey == 'labels') {
          ...
          await this.$store.dispatch('studies/updateLabelsFilterNoReload', {
            labels: labels,
          });
      ...
      for (const [key, value] of Object.entries(filters)) {
        if (key == 'labels') {
          ...
          this.filterStudyDateForDatePicker =
            resourceHelpers.parseDateForDatePicker(value);
    ...
    async loadStudiesFromChange(fromChangeId, limit) {
      ...
      for (let change of changes['Changes'].reverse()) {
        ...
        if (
          ...
          !this.latestStudiesIds.has(change['ID'])
        ) {
          ...
          try {
            ...
            if (
              this.latestStudiesIds.size == this.uiOptions.MaxStudiesDisplayed
            ) {
              this.isLoadingLatestStudies = false;
              this.isDisplayingLatestStudies = true;
              return;
      ...
      if (!this.shouldStopLoadingLatestStudies && fromChangeId > 0) {
        this.loadStudiesFromChange(
          Math.max(0, Math.max(0, fromChangeId - 1000)),
          1000,
        );
      } else {
        this.isLoadingLatestStudies = false;
    ...
    formatDateFromDatePicker(dates) {
      ...
      if (dates == null) {
        ...
        if (
          dates.length == 2 &&
          dates[0] != null &&
          dates[0].getFullYear != undefined &&
          dates[1] != null &&
          dates[1].getFullYear != undefined
        ) {
          if (
            resourceHelpers.toDicomDate(dates[0]) ==
            resourceHelpers.toDicomDate(dates[1])
          ) {
        ...
        <th v-for="columnTag in uiOptions.StudyListColumns" :key="columnTag">
          <div v-if="columnTag == 'StudyDate'" class="study-list-date-picker">
            <Datepicker
              ...
              range
              :preset-ranges="datePickerPresetRanges"
              format="yyyyMMdd"
              preview-format="yyyyMMdd"
              text-input
              arrow-navigation
        ...
        <tr class="actions-header">
          ...
          <td width="98%" colspan="10" scope="col" class="study-table-header">
            <div class="row g-1">
              ...
              <div class="col-7">
                <div
                  v-if="!isSearching && isLoadingLatestStudies"
                  class="alert alert-secondary study-list-alert"
                  role="alert"
                >
                  <span
                    v-if="isLoadingLatestStudies"
                ...
                <div
                  v-else-if="!isSearching && isDisplayingLatestStudies"
                  class="alert alert-secondary study-list-alert"
                  role="alert"
                >
